...
public:
    ...
    LQR_node(ros::NodeHandle& nh)//初始化中添加轨迹、小车初始位姿
	{
        ...
        sub_odom= nh.subscribe("/odom", 1, &LQR_node::odometryGetCallBack, this);        // 控制节点
        sub_globle_path = nh.subscribe("la2control", 1, &LQR_node::la_path_callback, this); // 接受local规划路径
    ...
    void la_path_callback(const la::path_la msg){
		...
		for(int i=0;i<msg.points.size();i++){
			...
			waypoint.ID = i;
			waypoint.x =msg.points[i].x;
			waypoint.y = msg.points[i].y;
			waypoint.yaw = msg.points[i].heading;
    ...
    void odometryGetCallBack(const nav_msgs::Odometry::ConstPtr odometry_msg){
        ...
        car.x= odometry_msg->pose.pose.position.x;
        car.y= odometry_msg->pose.pose.position.y;
        car.v=g_velocity;
        car.yaw=theta;
    }
	...
	void node_control() {
		...
		while (ros::ok()) {
			...
			br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "odom", "car"));
