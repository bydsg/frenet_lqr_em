   ...
   if (this->count == 0)
   {
    double raw, pitch, theta;
     ...
     tf::Matrix3x3(q).getRPY(raw, pitch, theta);
      std::string roadMap_path = "/home/by/code/lqr_by2/data/road_path5.txt";//地图录制位置
      FILE *fp_s;
      fp_s = fopen("/home/by/code/lqr_by2/data/road_path5.txt","a");
...
void path_record::globlepathGetCallBack(const by_djstl::Path msg){
      ...
      for(int i=0;i<msg.points.size();i++){
        ...
        if(first_flag==0){
        FILE *fp_s;
        fp_s = fopen("/home/by/code/lqr_by2/data/road_path6.txt", "a");
        fprintf(fp_s, "%lf %lf %lf", msg.points[i].x, msg.points[i].y, msg.points[i].heading); // Angle
        fprintf(fp_s, "\r\n");
        fclose(fp_s);
        }
